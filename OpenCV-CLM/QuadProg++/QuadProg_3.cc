...
/*************************************
 Modifications by Xiaoguang Yan:
 1. Changed functions to static.
 2. Changed solve_quadprog to solve_quadprog3
 ****************************************/
...
static bool add_constraint(double R[][MATRIX_DIM], double J[][MATRIX_DIM], double d[], int n, int& iq, double& rnorm);
static void delete_constraint(double R[][MATRIX_DIM], double J[][MATRIX_DIM], int A[], double u[], int n, int p, int& iq, int l);
...
static void cholesky_solve(double L[][MATRIX_DIM], double x[], double b[], int n);
...
double solve_quadprog_3(double G[][MATRIX_DIM], double g0[], int n, 
                      ...
                      double x[])
{
	int i, j, k, l; /* indices */
	int ip, me, mi; 
	double R[10][MATRIX_DIM], s[20], z[10], r[20], d[10], J[10][MATRIX_DIM], np[10], u[20];
	double x_old[10], u_old[20];
	double f_value, psi, c1, c2, sum, ss, R_norm;
	const double inf = std::numeric_limits<double>::infinity();
  double t, t1, t2; /* t is the step lenght, which is the minimum of the partial step length t1 
    * and the full step length t2 */
	int A[20], A_old[20], iai[20], q;
	int iq, iter = 0;
	bool iaexcl[20];
...
Q3_l2: /* Step 2: check for feasibility and determine a new S-pair */
	for (i = 0; i < mi; i++)
	{
...
Q3_l2a:/* Step 2a: determine step direction */
...
static bool add_constraint(double R[][MATRIX_DIM], double J[][MATRIX_DIM], double d[], int n, int& iq, double& R_norm)
{
#ifdef TRACE_SOLVER
