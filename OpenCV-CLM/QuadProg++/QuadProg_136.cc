...
/*************************************
 ...
 2. Changed solve_quadprog to solve_quadprog136
...
static bool add_constraint(double R[][MATRIX_DIM_440], double J[][MATRIX_DIM_440], double d[], int n, int& iq, double& rnorm);
static void delete_constraint(double R[][MATRIX_DIM_440], double J[][MATRIX_DIM_440], int A[], double u[], int n, int p, int& iq, int l);
...
static void cholesky_solve(double L[][MATRIX_DIM_440], double x[], double b[], int n);
                      ...
                      double x[])
{
	int i, j, k, l; /* indices */
	int ip, me, mi; 
	static double R[200][MATRIX_DIM_440], s[440], z[200], r[440], d[200], J[200][MATRIX_DIM_440], np[200], u[440];
	double x_old[200], u_old[440];
	double f_value, psi, c1, c2, sum, ss, R_norm;
	const double inf = std::numeric_limits<double>::infinity();
  double t, t1, t2; /* t is the step lenght, which is the minimum of the partial step length t1 
    * and the full step length t2 */
	int A[440], A_old[440], iai[440], q;
	int iq, iter = 0;
	bool iaexcl[440];
...
static bool add_constraint(double R[][MATRIX_DIM_440], double J[][MATRIX_DIM_440], double d[], int n, int& iq, double& R_norm)
{
#ifdef TRACE_SOLVER
